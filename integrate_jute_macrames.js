...
const generatedJsonPath = path.resolve('src/data/jute_macrames_generated.json');
...
console.log(`Loaded ${newProducts.length} new Jute Macrame products.`);
...
if (productsTsContent.includes(productsMarker)) {
  productsTsContent = productsTsContent.replace(
    productsMarker,
    `${productsMarker}  // ==========================================\n  // 6. Jute Macrames & Plant Hangers (Authentic Catalogue Collection - ${newProducts.length} Products)\n  // ==========================================\n${tsProductsCode},\n\n`
  );

  fs.writeFileSync(productsTsPath, productsTsContent, 'utf-8');
  console.log(`Successfully integrated ${newProducts.length} Jute Macrame products into src/data/products.ts!`);
} else {

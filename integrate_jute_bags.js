...
const generatedJsonPath = path.resolve('src/data/jute_bags_generated.json');
...
console.log(`Loaded ${newProducts.length} new Jute Bag products.`);
...
// Insert right after the Jute Baskets section or at the top of PRODUCTS array
...
if (productsTsContent.includes(productsMarker)) {
  productsTsContent = productsTsContent.replace(
    productsMarker,
    `${productsMarker}  // ==========================================\n  // 2. Jute Bags (Authentic Catalogue Collection - ${newProducts.length} Products)\n  // ==========================================\n${tsProductsCode},\n\n`
  );

  fs.writeFileSync(productsTsPath, productsTsContent, 'utf-8');
  console.log(`Successfully integrated ${newProducts.length} Jute Bag products into src/data/products.ts!`);
} else {

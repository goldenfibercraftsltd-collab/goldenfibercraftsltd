...
const generatedJsonPath = path.resolve('src/data/jute_poufs_generated.json');
...
console.log(`Loaded ${newProducts.length} new Jute Pouf products.`);
...
if (productsTsContent.includes(productsMarker)) {
  productsTsContent = productsTsContent.replace(
    productsMarker,
    `${productsMarker}  // ==========================================\n  // 5. Jute Poufs & Ottomans (Authentic Catalogue Collection - ${newProducts.length} Products)\n  // ==========================================\n${tsProductsCode},\n\n`
  );

  fs.writeFileSync(productsTsPath, productsTsContent, 'utf-8');
  console.log(`Successfully integrated ${newProducts.length} Jute Pouf products into src/data/products.ts!`);
} else {

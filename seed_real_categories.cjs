...
categories.forEach((cat) => {
  ...
  sql += `INSERT INTO categories (id, name, slug, description, icon, display_order, is_active, subcategories) VALUES (${cat.id}, '${escapedName}', '${cat.slug}', '${escapedDesc}', '${cat.icon}', ${cat.display_order}, 1, '${escapedSub}') ON CONFLICT(id) DO UPDATE SET name=excluded.name, slug=excluded.slug, description=excluded.description, icon=excluded.icon, display_order=excluded.display_order, is_active=1, subcategories=excluded.subcategories;\n`;
});

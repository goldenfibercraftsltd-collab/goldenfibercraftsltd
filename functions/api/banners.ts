...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  try {
    const result = await context.env.DB.prepare('SELECT * FROM banners WHERE is_active = 1 ORDER BY display_order ASC').all();
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    const { title, subtitle, image_url, link_url, display_order } = await context.request.json() as any;
    ...
    ).bind(title || '', subtitle || '', image_url, link_url || '', display_order || 0).run();

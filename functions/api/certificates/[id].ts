...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    for (const f of ['name', 'title', 'badge', 'logo_url', 'image_url', 'certificate_url', 'description', 'valid_until', 'display_order', 'is_active']) {
      if (body[f] !== undefined) { 
        fields.push(`${f} = ?`); 
    ...
    values.push(id);
    await context.env.DB.prepare(`UPDATE certificates SET ${fields.join(', ')} WHERE id = ?`).bind(...values).run();
...
export const onRequestDelete: PagesFunction<Env> = async (context) => {
  ...
  if (!user) return jsonResponse({ error: 'Unauthorized' }, 401);
  try {
    await context.env.DB.prepare('DELETE FROM certificates WHERE id = ?').bind(context.params.id).run();
    return jsonResponse({ success: true });
  } catch (err: any) { return jsonResponse({ error: err.message }, 500); }
};

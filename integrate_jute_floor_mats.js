...
const generatedJsonPath = path.resolve('src/data/jute_floor_mats_generated.json');
...
console.log(`Loaded ${newProducts.length} new Jute Floor Mat products.`);
...
// Insert right after the Jute Bags section or at top of PRODUCTS array
...
if (productsTsContent.includes(productsMarker)) {
  productsTsContent = productsTsContent.replace(
    productsMarker,
    `${productsMarker}  // ==========================================\n  // 3. Jute Floor Mats (Authentic Catalogue Collection - ${newProducts.length} Products)\n  // ==========================================\n${tsProductsCode},\n\n`
  );

  fs.writeFileSync(productsTsPath, productsTsContent, 'utf-8');
  console.log(`Successfully integrated ${newProducts.length} Jute Floor Mat products into src/data/products.ts!`);
} else {
